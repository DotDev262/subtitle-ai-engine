...
class IndicTransTranslator:
    ...
    def _generate(
        ...
        formatted = [f"{self.src_lang} {self.tgt_lang} {text}" for text in texts]
        inputs = self.tokenizer(
            formatted, padding="longest", truncation=True, max_length=256, return_tensors="pt"
        ).to(self.device)
        gen_kwargs = dict(
            num_beams=num_beams,
    ...
    def translate_constrained(self, text: str, max_cpl: int = 42) -> str:
        processor = CPLLogitsProcessor(
            max_cpl=max_cpl, tokenizer=self.tokenizer, eos_token_id=self.eos_id
        )
